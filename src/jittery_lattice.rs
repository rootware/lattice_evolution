...
impl JitteryLattice {
    ...
    pub fn step( &mut self, amplitude: f64, omega: f64){
        ...
        while it < no_iter {
            ...
            self.rk4step( dt,  amplitude, omega, time, noise);
            it+=1; time+=dt;
        }
    ...
    pub fn update(&self, wavefunction: DVector<Complex64>, amplitude:f64, omega: f64, t: f64 , noise: f64) -> DVector<Complex64> {
        let phi = amplitude*f64::sin(omega*t);
        let hamiltonian = &self.h0 + &self.h1*Complex64::from(f64::sin(phi))*Complex64::from(1.0+noise) - &self.h2*Complex64::from(f64::cos(phi))*Complex64::from(1.0+noise);
        hamiltonian*wavefunction*Complex64::new(0.0,-1.0)
    }

    /// The RK4 step.
    pub fn rk4step(&mut self, dt:f64, amplitude: f64, omega: f64, t:f64, noise : f64){
        let k1 = self.update(self.psi.clone(), amplitude, omega, t, noise);
        let k2 = self.update(self.psi.clone() + &k1*Complex64::from(dt/2.0) , amplitude, omega, t+dt/2.0, noise);
        let k3 = self.update(self.psi.clone() + &k2*Complex64::from(dt/2.0) , amplitude, omega, t+dt/2.0, noise);
        let k4 = self.update(self.psi.clone() + &k3*Complex64::from(dt) , amplitude, omega, t+dt, noise);
...
impl JitteryLattice {
    ...
    pub fn new(acceleration: f64, latticedepth: f64, jittersize: f64) -> JitteryLattice {
        let max_p: f64 = (N_STATES as f64)-1.0 ; 
